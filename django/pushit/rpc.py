...
def on_git_push_request(info, request):
    state = request.DATA.get("object_attributes.state")
    target_branch = request.DATA.get("object_attributes.target_branch")
    ...
    branch = request.DATA.get("ref").split('/')[-1]
    if branch == git_branch:
        reportRestIssue("{} update({}) on: {}".format(info.get("project", "n/a"), getattr(settings, "SERVER_NAME", "unknown"), git_branch), "<pre>{}</pre>".format(helpers.dictToString(request.DATA.asDict(), True)), "git_updates", email_only=True)
        update_code(git_updater, git_updater_md5, git_branch)
...
def getRelease(request):
    ...
    if sec_key != git_key:
        sec_key = request.DATA.getHeader("HTTP_X_GITLAB_TOKEN")
        if not sec_key:
