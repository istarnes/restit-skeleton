...
class PersistentLog(models.Model, RestModel):
    class RestMeta:
        DEFAULT_SORT = "-when"
        CAN_SAVE = False
        QUERY_FIELDS = ["level", "component", "pkey", "action", "request_method", "request_path", "group"]
    ...
    def log(message, level=0, request=None, component=None,
            pkey=None, action=None, group=None, path=None, method=None, no_truncate=False):
        plog = PersistentLog.createLogFromRequest(
            request, component=component, pkey=pkey, action=action, group=group,
    ...
    def logException(message=None, level=50, request=None, component=None,
        ...
        if request:
            ...
            # request.DATA.log()
            helpers.log_error("path={}\nip={}\nuser={}".format(
                path, request.ip, request.user))

        # LOG THE EXCEPTION TO local LOGGERS
        plog = PersistentLog.createLogFromRequest(
            request=request, component=component, pkey=pkey, action=action, group=group,
            path=path, method=method, level=level)
    ...
    @staticmethod
    def createLogFromRequest(request=None, component=None,
        ...
        plog = PersistentLog(
            group=group, level=level, component=component,
