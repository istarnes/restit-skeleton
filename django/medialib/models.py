...
class MediaItem(models.Model, RestModel, MetaDataModel):
    ...
    def on_rest_list_filter(cls, request, qset=None):
        # to view media items one must be the owner or in a group
        if request.group:
            if request.group.isMember(request.member):
                qset = qset.filter(group=request.group)
            else:
                qset = qset.none()
        else:
            # you must supply a group to get media items for
            qset = qset.filter(owner=request.member)
        return qset
